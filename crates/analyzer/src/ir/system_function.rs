...
use crate::ir::{AssignDestination, Comptime, Expression, IrResult, Type, TypeKind, VarPathSelect};
    ...
    pub fn new(
        ...
        match name.to_string().as_str() {
            ...
            "$clog2" => {
                ...
                let arg0_type = Type::new(TypeKind::Logic, vec![], vec![32], false);
    ...
    pub fn eval_comptime(&self, context: &mut Context) -> Comptime {
        ...
        match &self.kind {
            SystemFunctionKind::Bits(x) => {
                ...
                Comptime::create_value(value.into(), 32, ClockDomain::None, self.token)
            }
            SystemFunctionKind::Clog2(_) => {
                if let Some(x) = value {
                    Comptime::create_value(x.payload, 32, ClockDomain::None, self.token)
                } else {
            ...
            SystemFunctionKind::Onehot(_) => {
                if let Some(x) = value {
                    Comptime::create_value(x.payload, 1, ClockDomain::None, self.token)
                } else {
                    Comptime::create_unknown(ClockDomain::None, self.token)
                }
            }
