...
use crate::ir::{AssignDestination, Comptime, Expression, IrResult, Type, TypeKind};
...
fn create_input(
    ...
    name: StrId,
    r#type: Option<Type>,
    arg: (Expression, Vec<AssignDestination>, TokenRange),
) -> Input {
    let (expr, _, token) = arg;

    if let Some(r#type) = r#type {
        let comptime = expr.eval_comptime(context, None);
        if !comptime.r#type.compatible(&r#type) {
...
impl SystemFunctionCall {
    pub fn new(
        context: &mut Context,
        name: StrId,
        mut args: Vec<(Expression, Vec<AssignDestination>, TokenRange)>,
        token: TokenRange,
    ) -> IrResult<Self> {
        match name.to_string().as_str() {
            ...
            "$clog2" => {
                ...
                let arg0_type = Type::new(TypeKind::Bit, vec![], vec![32], false);
    ...
    pub fn eval_value(&self, context: &mut Context) -> Option<Value> {
        match &self.kind {
            SystemFunctionKind::Bits(_) => None,
    ...
    pub fn eval_comptime(&self, context: &mut Context) -> Comptime {
        ...
        match &self.kind {
            SystemFunctionKind::Bits(x) => {
                let comptime = x.0.eval_comptime(context, None);
                let value = comptime.r#type.total_width();
                Comptime::create_value(value.into(), 32, self.token)
            }
            ...
            SystemFunctionKind::Readmemh(_, _) => Comptime::create_unknown(self.token),
        }
    }
