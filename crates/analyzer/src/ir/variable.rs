        ...
        let ret: Vec<_> = ret
            ...
            .map(|x| {
                Expression::Term(Box::new(Factor::Value(
                    Comptime::create_value(x.into(), 32, token),
                    token,
                )))
            })
            .collect();
...
impl VarSelect {
    ...
    pub fn eval_value(&self, context: &mut Context, r#type: &[usize]) -> Option<(usize, usize)> {
        if self.0.is_empty() {
...
mod tests {
    ...
    fn gen_var_select(x: &[u32], y: Option<u32>) -> VarSelect {
        ...
        for x in x {
            let token = TokenRange::default();
            let value = Comptime::create_value((*x).into(), 8, token);
        ...
        if let Some(y) = y {
            let token = TokenRange::default();
            let value = Comptime::create_value(y.into(), 8, token);
            let factor = Factor::Value(value, token);
            let expr = Expression::Term(Box::new(factor));
    ...
    fn var_select_array() {
        ...
        let z0 = x0.eval_value(&mut context, &[4, 5, 6]).unwrap();
        let z1 = x1.eval_value(&mut context, &[4, 5, 6]).unwrap();
        let z2 = x2.eval_value(&mut context, &[4, 5, 6]).unwrap();
        let z3 = x3.eval_value(&mut context, &[4, 5, 6]).unwrap();
        let z4 = x4.eval_value(&mut context, &[4, 5, 6]).unwrap();
        let z5 = x5.eval_value(&mut context, &[4, 5, 6]).unwrap();
    ...
    fn var_select_width() {
        ...
        let z4 = x4.eval_value(&mut context, &[4, 5, 6]).unwrap();
        let z5 = x5.eval_value(&mut context, &[4, 5, 6]).unwrap();
        let z6 = x6.eval_value(&mut context, &[4, 5, 6]).unwrap();
