...
use crate::ir::{AssignDestination, Comptime, Expression, Factor, Type};
...
impl VarIndex {
        ...
        let ret: Vec<_> = ret
            ...
            .map(|x| {
                Expression::Term(Box::new(Factor::Value(
                    Comptime::create_value(x.into(), 32, ClockDomain::None, token),
                    token,
                )))
            })
            .collect();
    ...
    pub fn is_const(&self, context: &mut Context) -> bool {
        ...
        for x in &self.0 {
            let comptime = x.eval_comptime(context, None);
...
impl VarSelect {
    ...
    pub fn eval_comptime(
        ...
        if self.is_empty() {
            ...
            let beg = beg.eval_value(context, None)?.to_usize();
...
impl Variable {
    ...
    pub fn get_value(&self, index: &[usize]) -> Option<Value> {
        let index = calc_index(index, &self.r#type.array)?;
        self.value.get(index).cloned()
    }

    pub fn set_value(&mut self, index: &[usize], mut value: Value) -> bool {
        let Some(index) = calc_index(index, &self.r#type.array) else {
            return false;
        };
        value.trunc(self.total_width());
    ...
    pub fn prepend_array(&mut self, array: &[usize]) {
        if !array.is_empty() {
            let total_array: usize = array.iter().product();
            let value = self.value.clone();
            let assigned = self.assigned.clone();
            for _ in 0..total_array - 1 {
...
mod tests {
    ...
    fn gen_var_select(x: &[u32], y: Option<u32>) -> VarSelect {
        ...
        for x in x {
            let token = TokenRange::default();
            let value = Comptime::create_value((*x).into(), 8, ClockDomain::None, token);
            let factor = Factor::Value(value, token);
        ...
        if let Some(y) = y {
            let token = TokenRange::default();
            let value = Comptime::create_value(y.into(), 8, ClockDomain::None, token);
            let factor = Factor::Value(value, token);
            let expr = Expression::Term(Box::new(factor));
    ...
    fn var_select_array() {
        ...
        let y0 = x0.eval_comptime(&mut context, &[4, 5, 6], true).unwrap();
        let y1 = x1.eval_comptime(&mut context, &[4, 5, 6], true).unwrap();
        let y2 = x2.eval_comptime(&mut context, &[4, 5, 6], true).unwrap();
        let y3 = x3.eval_comptime(&mut context, &[4, 5, 6], true).unwrap();
        let y4 = x4.eval_comptime(&mut context, &[4, 5, 6], true).unwrap();
        let y5 = x5.eval_comptime(&mut context, &[4, 5, 6], true).unwrap();
        ...
        let z0 = x0.eval_value(&mut context, &[4, 5, 6], true).unwrap();
        let z1 = x1.eval_value(&mut context, &[4, 5, 6], true).unwrap();
        let z2 = x2.eval_value(&mut context, &[4, 5, 6], true).unwrap();
        let z3 = x3.eval_value(&mut context, &[4, 5, 6], true).unwrap();
        let z4 = x4.eval_value(&mut context, &[4, 5, 6], true).unwrap();
        let z5 = x5.eval_value(&mut context, &[4, 5, 6], true).unwrap();
    ...
    fn var_select_width() {
        ...
        let y0 = x0.eval_comptime(&mut context, &[4, 5, 6], false).unwrap();
        let y1 = x1.eval_comptime(&mut context, &[4, 5, 6], false).unwrap();
        let y2 = x2.eval_comptime(&mut context, &[4, 5, 6], false).unwrap();
        let y3 = x3.eval_comptime(&mut context, &[4, 5, 6], false).unwrap();
        let y4 = x4.eval_comptime(&mut context, &[4, 5, 6], false).unwrap();
        let y5 = x5.eval_comptime(&mut context, &[4, 5, 6], false).unwrap();
        let y6 = x6.eval_comptime(&mut context, &[4, 5, 6], false).unwrap();
        ...
        let z0 = x0.eval_value(&mut context, &[4, 5, 6], false).unwrap();
        let z1 = x1.eval_value(&mut context, &[4, 5, 6], false).unwrap();
        let z2 = x2.eval_value(&mut context, &[4, 5, 6], false).unwrap();
        let z3 = x3.eval_value(&mut context, &[4, 5, 6], false).unwrap();
        let z4 = x4.eval_value(&mut context, &[4, 5, 6], false).unwrap();
        let z5 = x5.eval_value(&mut context, &[4, 5, 6], false).unwrap();
        let z6 = x6.eval_value(&mut context, &[4, 5, 6], false).unwrap();
