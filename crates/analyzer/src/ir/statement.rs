...
use crate::ir::{
    Comptime, Expression, FunctionCall, IrResult, SystemFunctionCall, VarId, VarIndex, VarPath,
    VarSelect,
};
...
impl AssignDestination {
    pub fn expand(&self, context: &mut Context) -> IrResult<Vec<AssignDestination>> {
        if self.comptime.r#type.is_struct() {
            let mut ret = vec![];
            for x in self.comptime.r#type.expand_struct(&self.path) {
                let (id, comptime) = context
                    .find_path(&x.path)
                    .ok_or_else(|| ir_error!(self.token))?;
                ret.push(AssignDestination {
                    id,
                    path: x.path,
    ...
    pub fn total_width(&self, context: &mut Context) -> Option<usize> {
        let (beg, end) = self
            .select
            .eval_value(context, &self.comptime.r#type.width, false)?;
        Some(beg - end + 1)
    }
    ...
    pub fn eval_assign(
        ...
        if let Some(index) = self.index.eval_value(context)
            ...
            && let Some((beg, end)) = self
                .select
                .eval_value(context, &variable.r#type.width, false)
        {
            let mask = gen_mask_range(beg, end);
            let mut errors = vec![];

            let is_const = self.index.is_const(context);

            // If index is empty or not const, assign to the whole array
            if index.is_empty() | !is_const {
                for i in 0..variable.r#type.total_array() {
                    ...
                    if let Some(index) = index.eval_value(context) {
                        ...
                        {
                            errors.push(AnalyzerError::unassign_variable(
                ...
                let (success, tokens) =
                    assign_table.insert_assign(&variable, index, mask, maybe, self.token);
...
impl IfStatement {
    ...
    pub fn eval_assign(
        ...
        assign_table.merge_by_or(context, &mut true_table, assign_context.is_ff());
