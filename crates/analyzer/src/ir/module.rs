...
use crate::ir::utils::total_array;
    ...
    pub fn eval_assign(&self, context: &mut Context) {
        ...
        for (key, entry) in &assign_table.table {
            if let Some(variable) = variables.get_mut(key) {
                for i in 0..total_array(&entry.array) {
        ...
        for variable in variables.values() {
            // skip to check systemverilog type
            if variable.is_assignable() && !variable.r#type.is_systemverilog() {
