...
impl Comptime {
    ...
    pub fn invalid_operand2(&mut self, context: &mut Context, op: Op, x: &Type) {
        context.insert_error(AnalyzerError::invalid_operand(
            &x.to_string(),
            &op.to_string(),
            &self.token,
    ...
    pub fn invalid_logical_operand2(&mut self, context: &mut Context) {
        context.insert_error(AnalyzerError::invalid_logical_operand(true, &self.token));
        self.value = ValueVariant::Unknown;
        self.r#type = Type {
            kind: TypeKind::Unknown,
            ..Default::default()
