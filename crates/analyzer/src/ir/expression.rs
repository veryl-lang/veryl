...
use crate::value::{Value, gen_mask, to_biguint};
...
impl Expression {
    ...
    pub fn eval_comptime(&self, context: &mut Context, context_width: Option<usize>) -> Comptime {
        ...
        match self {
            ...
            Expression::Concatenation(x) => {
                let mut ret = Comptime::create_unknown(self.token_range());
                ret.value = value;
                ...
                for (expr, repeat) in x {
                    ...
                    if let Some(repeat) = repeat {
                        if let Some(repeat) = eval_repeat(context, repeat) {
                            ...
                            return Comptime::create_unknown(self.token_range());
                        }
                    } else {
            ...
            // ArrayLiteral doesn't require evaluation because it is expanded in conv phase
            Expression::ArrayLiteral(_) => Comptime::create_unknown(self.token_range()),
        }
    }
    ...
    pub fn eval_value(
        ...
        match self {
            Factor::Variable(id, index, select, comptime, _) => {
                ...
                if select.is_range() {
                    let (beg, end) = select.eval_value(context, &comptime.r#type.width)?;
...
mod tests {
    ...
    fn calc_expression(s: &str, context_width: Option<usize>) -> Value {
        ...
        let x: Expression = Conv::conv(&mut Context::default(), &x).unwrap();
