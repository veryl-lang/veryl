    ...
    pub fn eval_value(
        ...
        match self {
            ...
            Factor::Struct(ids, index, select, comptime, _) => {
                ...
                if !select.is_empty() {
                    let width = Shape::new(vec![comptime.r#type.total_width()]);
                    let mut r#type = Type::create_unknown();
                    r#type.width = width;
    ...
    pub fn eval_comptime(
        ...
        match self {
            ...
            Factor::Struct(_, index, select, comptime, _) => {
                ...
                if !select.is_empty() {
                    // create flatten type for select.eval_comptime
                    let mut r#type = ret.r#type.clone();
                    r#type.width = Shape::new(vec![ret.r#type.total_width()]);

                    if let Some(width) = select.eval_comptime(context, &r#type, false) {
                        ret.r#type.kind = if ret.r#type.is_2state() {
                            TypeKind::Bit
                        } else {
                            TypeKind::Logic
                        };
