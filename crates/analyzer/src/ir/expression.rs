...
use crate::ir::bigint::{gen_mask, to_biguint};
use crate::ir::{
    FunctionCall, SystemFunctionCall, Type, TypeKind, TypedValue, Value, ValueVariant, VarId,
...
use veryl_parser::veryl_token::is_anonymous_token;
...
impl Expression {
    pub fn eval(
        ...
        match self {
            Expression::Term(x) => x.eval(context_width, map),
            ...
            Expression::Binary(x, op, y) => {
                ...
                let ret = match op {
                    ...
                    Op::Div => binary_op_signed(
                        ...
                        |x, y| x.checked_div(&y).unwrap_or(0u32.into()),
                        |x, y| x & y,
                    ),
                    Op::Rem => binary_op_signed(
                        ...
                        context_width,
                        |x, y, z| x.max(y).max(z.unwrap_or(0)),
                        |x, y| if y == 0u32.into() { 0u32.into() } else { x % y },
                        |x, y| x & y,
                    ),
                    ...
                    Op::ArithShiftR => {
                        let width = x.width;
                        let sign = x.payload.bit(width as u64 - 1);
                        let mask = if sign {
                            let shift = y.payload.to_u32().unwrap() as usize;
                        ...
                        binary_op(
                            ...
                            context_width,
                            |x, _, z| x.max(z.unwrap_or(0)),
                            |x, y| (x >> y.to_u32().unwrap()) | mask.clone(),
                            |x, y| x & y,
                        )
                    }
    ...
    pub fn eval_type(&self, context: &mut Context, context_width: Option<usize>) -> TypedValue {
        let value = self.eval(context_width, &context.variables);
        ...
        match self {
            Expression::Term(x) => x.eval_type(context, context_width),
            Expression::Unary(op, x) => {
                let range = x.token_range();
                let x = x.eval_type(context, context_width);
                let mut ret = x.clone();
                ret.value = value;
            ...
            Expression::Binary(x, op, y) => {
                ...
                let x = x.eval_type(context, context_width);
                let y = y.eval_type(context, context_width);
                ...
                let width = match op {
                    ...
                    | Op::ArithShiftR
                    | Op::LogicShiftL
                    | Op::LogicShiftR => x_width.max(context_width.unwrap_or(0)),
                    Op::Div
                    | Op::Rem
                    | Op::Mul
                    | Op::Add
                    ...
                    | Op::BitOr
                    | Op::BitXor
                    | Op::BitXnor => x_width.max(y_width).max(context_width.unwrap_or(0)),
                    Op::LessEq
                    | Op::GreaterEq
                    | Op::Less
                    | Op::Greater
                ...
                match op {
                    ...
                    | Op::LogicAnd
                    | Op::LogicOr => {
                        if x.r#type.is_2state() && y.r#type.is_2state() {
                    ...
                    Op::As => {
                        if let ValueVariant::Type(x) = y.value {
                            ret.r#type = x;
                        } else {
                            ret.invalid_operand(context, *op, &y.r#type, &range_y);
                            return ret;
            ...
            Expression::Ternary(x, y, z) => {
                ...
                let x = x.eval_type(context, context_width);
                let y = y.eval_type(context, context_width);
                let z = z.eval_type(context, context_width);
                ...
                if y.r#type.is_2state() && z.r#type.is_2state() {
                    ret.r#type.kind = TypeKind::Bit;
                } else {
                    ret.r#type.kind = TypeKind::Logic;
                }
            ...
            Expression::Concatenation(x) => {
                let mut ret = TypedValue::create_unknown();
                ret.value = value;
                ...
                for (expr, repeat) in x {
                    ...
                    let expr = expr.eval_type(context, context_width);
                    ...
                    if let Some(repeat) = repeat {
                        if let Some(repeat) = eval_repeat(context, repeat) {
                            width += expr.r#type.total_width() * repeat.to_usize();
                        } else {
                            return TypedValue::create_unknown();
                        }
                    } else {
                        width += expr.r#type.total_width();
                    }
            ...
            // ArrayLiteral doesn't require evaluation because it is expanded in conv phase
            Expression::ArrayLiteral(_) => TypedValue::create_unknown(),
        }
    }
    ...
    pub fn eval_assign(
        ...
        match self {
            Expression::Term(x) => x.eval_assign(context, assign_table, in_comb),
            Expression::Unary(_, x) => x.eval_assign(context, assign_table, in_comb),
            Expression::Binary(x, _, y) => {
                x.eval_assign(context, assign_table, in_comb);
                y.eval_assign(context, assign_table, in_comb);
            }
            Expression::Ternary(x, y, z) => {
                x.eval_assign(context, assign_table, in_comb);
                y.eval_assign(context, assign_table, in_comb);
                z.eval_assign(context, assign_table, in_comb);
            }
            Expression::Concatenation(x) => {
                for (x, y) in x {
                    x.eval_assign(context, assign_table, in_comb);
    ...
    pub fn rename(&mut self, table: &HashMap<VarId, VarId>) {
        match self {
            Expression::Term(x) => x.rename(table),
            Expression::Unary(_, x) => x.rename(table),
            Expression::Binary(x, _, y) => {
                x.rename(table);
                y.rename(table);
            }
...
#[derive(Clone, Debug)]
pub enum Factor {
    Variable(VarId, VarIndex, VarSelect, TypedValue, TokenRange),
    Value(TypedValue, TokenRange),
...
impl Factor {
    pub fn eval(
        ...
        match self {
            Factor::Variable(id, index, select, typed_value, _) => {
                let index = index.eval(map)?;
                let value = map.get(id)?.get_value(&index)?;

                if select.is_range() {
                    let (beg, end) = select.eval(map, &typed_value.r#type.width)?;
                    Some(value.select(beg, end))
                } else {
                    Some(value)
                }
            }
            Factor::Value(x, _) => x.get_value(),
            Factor::SystemFunctionCall(x, _) => {
                let mut args = vec![];
                for x in &x.args {
                    args.push(x.eval(None, map));
                }
                x.kind.eval(&args)
            }
    ...
    pub fn eval_type(&self, context: &mut Context, context_width: Option<usize>) -> TypedValue {
        match self {
            Factor::Variable(_, index, select, typed_value, _) => {
                let mut ret = typed_value.clone();
                let value = self.eval(context_width, &context.variables);
                let value = if let Some(x) = value {
                    ValueVariant::Numeric(x)
                } else {
                ...
                if let Some(width) = select.eval_type(context, &ret.r#type.width, false) {
            ...
            Factor::SystemFunctionCall(x, y) => {
                let mut args = vec![];
                for x in &x.args {
                    args.push(x.eval_type(context, None));
                }
                x.kind.eval_type(context, &args, y)
            }
            Factor::FunctionCall(x, _) => {
                if let Some(x) = &x.ret {
                    x.clone()
                } else {
                    TypedValue::create_unknown()
                }
            }
            Factor::Unresolved(x, _) => {
                let is_global = is_anonymous_token(&x.identifier().token);
    ...
    pub fn eval_assign(
        ...
        if let Factor::FunctionCall(x, _) = self {
            x.eval_assign(context, assign_table, in_comb);
        }
    }
    ...
    pub fn rename(&mut self, table: &HashMap<VarId, VarId>) {
        match self {
            Factor::Variable(x, _, _, _, _) => {
                if let Some(id) = table.get(x) {
                    *x = *id;
                }
            }
            Factor::FunctionCall(x, _) => x.rename(table),
            _ => (),
        }
    }
...
mod tests {
    ...
    fn calc_expression(
        ...
        let x: Expression = Conv::conv(&mut Context::default(), &x);
        x.eval(context_width, map).unwrap()
    }
    ...
    fn arithmetic() {
        ...
        let x00 = calc_expression("1 + 2", None, &map);
        let x01 = calc_expression("5 - 1", None, &map);
        let x02 = calc_expression("2 * 7", None, &map);
        let x03 = calc_expression("8 / 3", None, &map);
        let x04 = calc_expression("9 % 4", None, &map);
        let x05 = calc_expression("2 ** 3", None, &map);
        let x06 = calc_expression("+5", None, &map);
        let x07 = calc_expression("-1", None, &map);
        let x08 = calc_expression("1 << 2", None, &map);
        let x09 = calc_expression("1 <<< 2", None, &map);
        let x10 = calc_expression("-8 >> 2", None, &map);
        let x11 = calc_expression("-8 >>> 2", None, &map);
    ...
    fn eval_type_unary(context: &mut Context, op: Op, x: Box<Expression>) -> TypedValue {
        let ret = Expression::Unary(op, x);
        ret.eval_type(context, None)
    }
    ...
    fn eval_type_binary(
        ...
        let ret = Expression::Binary(x, op, y);
        ret.eval_type(context, None)
    }

    fn eval_type_ternary(
        context: &mut Context,
        x: Box<Expression>,
        y: Box<Expression>,
        z: Box<Expression>,
    ) -> TypedValue {
        let ret = Expression::Ternary(x, y, z);
        ret.eval_type(context, None)
    }
    ...
    fn eval_type_concat(
        ...
        let ret = Expression::Concatenation(x);
        ret.eval_type(context, None)
    }
    ...
    fn unary_type() {
        ...
        let x0 = eval_type_unary(&mut context, Op::BitAnd, bit(3));
        let x1 = eval_type_unary(&mut context, Op::BitAnd, logic(4));
        let x2 = eval_type_unary(&mut context, Op::BitNot, bit(3));
        let x3 = eval_type_unary(&mut context, Op::BitNot, logic(4));
        let x4 = eval_type_unary(&mut context, Op::LogicNot, bit(1));
        let x5 = eval_type_unary(&mut context, Op::LogicNot, logic(1));
        let x6 = eval_type_unary(&mut context, Op::Add, bit(3));
        let x7 = eval_type_unary(&mut context, Op::Add, logic(4));
    ...
    fn binary_type() {
        ...
        let x00 = eval_type_binary(&mut context, bit(4), Op::Add, bit(3));
        let x01 = eval_type_binary(&mut context, bit(4), Op::Add, logic(3));
        let x02 = eval_type_binary(&mut context, logic(4), Op::Add, logic(3));
        let x03 = eval_type_binary(&mut context, bit(4), Op::Eq, bit(3));
        let x04 = eval_type_binary(&mut context, bit(4), Op::Eq, logic(3));
        let x05 = eval_type_binary(&mut context, logic(4), Op::Eq, logic(3));
        let x06 = eval_type_binary(&mut context, bit(1), Op::LogicAnd, bit(1));
        let x07 = eval_type_binary(&mut context, bit(1), Op::LogicAnd, logic(1));
        let x08 = eval_type_binary(&mut context, logic(1), Op::LogicAnd, logic(1));
        let x09 = eval_type_binary(&mut context, bit(4), Op::ArithShiftL, bit(3));
        let x10 = eval_type_binary(&mut context, bit(4), Op::ArithShiftL, logic(3));
        let x11 = eval_type_binary(&mut context, logic(4), Op::ArithShiftL, logic(3));
    ...
    fn ternary_type() {
        ...
        let x0 = eval_type_ternary(&mut context, bit(1), bit(4), bit(3));
        let x1 = eval_type_ternary(&mut context, bit(1), bit(4), logic(3));
        let x2 = eval_type_ternary(&mut context, bit(1), logic(4), bit(3));
        let x3 = eval_type_ternary(&mut context, bit(1), logic(4), logic(3));
        let x4 = eval_type_ternary(&mut context, logic(1), bit(4), bit(3));
        let x5 = eval_type_ternary(&mut context, logic(1), bit(4), logic(3));
        let x6 = eval_type_ternary(&mut context, logic(1), logic(4), bit(3));
        let x7 = eval_type_ternary(&mut context, logic(1), logic(4), logic(3));
