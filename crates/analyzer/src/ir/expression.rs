...
use crate::ir::{
    Comptime, FunctionCall, SystemFunctionCall, Type, TypeKind, ValueVariant, VarId, VarIndex,
    VarSelect,
};
...
impl Expression {
    ...
    pub fn eval_value(&self, context: &mut Context, context_width: Option<usize>) -> Option<Value> {
        match self {
            ...
            Expression::Concatenation(x) => {
                ...
                for (exp, rep) in x.iter() {
                    ...
                    let rep = if let Some(rep) = rep {
                        ...
                        let rep = rep.payload.to_usize().unwrap();
            ...
            // ArrayLiteral/StructConstructor doesn't require evaluation because it is expanded in conv phase
            Expression::ArrayLiteral(_) => None,
            Expression::StructConstructor(_, _) => None,
        }
    }

    pub fn eval_comptime(&self, context: &mut Context, context_width: Option<usize>) -> Comptime {
        ...
        match self {
                ...
                match op {
                    ...
                    Op::As => {
                        if let ValueVariant::Type(y) = y.value {
                            ...
                            let invalid_reset_cast = y.is_reset() && !x.r#type.is_reset();
            ...
            Expression::Concatenation(x) => {
                let mut ret = Comptime::create_unknown(ClockDomain::None, self.token_range());
                ret.value = value;
                ...
                for (expr, repeat) in x {
                    ...
                    check_clock_domain(context, &ret, &expr, &self.token_range().beg);
                    ...
                    if let Some(repeat) = repeat {
                        if let Some(repeat) = eval_repeat(context, repeat) {
                            ...
                            return Comptime::create_unknown(ClockDomain::None, self.token_range());
                        }
                    } else {
            ...
            Expression::StructConstructor(_, exprs) => {
                ...
                let mut is_global = true;
                for expr in exprs {
                    let comptime = expr.1.eval_comptime(context, None);
    ...
    pub fn eval_assign(
        ...
        match self {
            ...
            Expression::StructConstructor(_, _) => (),
        }
    }
...
pub enum Factor {
    ...
    Struct(Vec<VarId>, VarIndex, VarSelect, Comptime, TokenRange),
    ...
    pub fn eval_value(
        ...
        match self {
            Factor::Variable(id, index, select, comptime, _) => {
                ...
                if select.is_range() {
                    let (beg, end) = select.eval_value(context, &comptime.r#type.width, false)?;
                    Some(value.select(beg, end))
                } else {
                    Some(value)
                }
            }
            Factor::Struct(ids, index, select, comptime, _) => {
                let index = index.eval_value(context)?;
                let mut value = Value::new(0u32.into(), 0, false);
                for id in ids {
                    let x = context.variables.get(id)?.get_value(&index)?;
    ...
    pub fn eval_comptime(&self, context: &mut Context, context_width: Option<usize>) -> Comptime {
        match self {
            Factor::Variable(_, index, select, comptime, _) => {
                ...
                // Enum should be treated as flatten bit/logic when it is bit-selected
                if !select.is_empty() {
                    ret.r#type.flatten_enum()
                }
            ...
            Factor::Struct(_, index, select, comptime, _) => {
                let mut ret = comptime.clone();
                let value = self.eval_value(context, context_width);
                let value = if let Some(x) = value {
                    ValueVariant::Numeric(x)
                ...
                // create flatten type for eval_comptime
                let mut r#type = ret.r#type.clone();
                r#type.width = vec![ret.r#type.total_width()];

                if let Some(width) = select.eval_comptime(context, &r#type, false) {
                    ret.r#type.width = width;
                }
        ...
        match self {
            Factor::Variable(id, index, select, _, _) => {
                if let Some(index) = index.eval_value(context)
                    ...
                    && let Some((beg, end)) =
                        select.eval_value(context, &variable.r#type.width, false)
                {
...
impl fmt::Display for Factor {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let ret = match self {
            ...
            Factor::Struct(ids, index, select, _, _) => {
                let mut ret = String::new();
                for id in ids {
                    ret.push_str(&format!(", {id}"));
                }
                format!("{{{}}}{index}{select}", &ret[2..])
            }
            Factor::Value(x, _) => {
...
impl ArrayLiteralItem {
    ...
    pub fn is_global(&self, context: &mut Context) -> bool {
        match self {
            ArrayLiteralItem::Value(x, y) => {
...
mod tests {
    ...
    fn calc_expression(s: &str, context_width: Option<usize>) -> Value {
        ...
        context.config.evaluate_size_limit = 1024 * 1024;
