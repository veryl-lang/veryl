...
use crate::ir::{Type, TypeKind, TypedValue, Value, ValueVariant, VarId, VarIndex, Variable};
        ...
        match self {
            ...
            Expression::Concatenation(x) => {
                ...
                for (x, y) in x {
                    ...
                    // array / type can't be operated
                    if x.r#type.is_array() | x.r#type.is_type() {
                        ret.invalid_operand(context, Op::Concatenation, &x.r#type, &range);
                        return ret;
                    }

                    if x.r#type.is_4state() {
                        kind = TypeKind::Logic;
                    }

                    if let Some(y) = y {
                        let range = y.token_range();
                        let y = y.eval_type(context, context_width);

                        // array / type can't be operated
                        if y.r#type.is_array() | y.r#type.is_type() {
                            ret.invalid_operand(context, Op::Concatenation, &y.r#type, &range);
                            return ret;
                        }

                        if let ValueVariant::Numeric(y) = y.value {
                            width += x.r#type.total_width() * y.to_usize();
                        } else {
                            ret.invalid_operand(context, Op::Concatenation, &y.r#type, &range);
                            return ret;
                        }
...
#[derive(Clone, Debug)]
pub enum Factor {
    Variable(VarId, VarIndex, TypedValue, Vec<Select>, TokenRange),
...
impl Factor {
    pub fn eval(
        ...
        match self {
            Factor::Variable(x, i, _, y, _) => {
                let index = i.eval(map)?;
                let value = map.get(x)?.get_value(&index)?;

                // TODO multi-dimentional packed array
                if let Some(y) = y.last() {
    ...
    pub fn eval_type(&self, context: &mut Context, context_width: Option<usize>) -> TypedValue {
        match self {
            Factor::Variable(_, i, x, y, _) => {
                ...
                ret.r#type.array.drain(0..i.dimension());

                // TODO multi-dimentional packed array
                if let Some(y) = y.last()
                    && let Some(beg) = y.beg.eval(context_width, &context.variables)
                {
                    let end = if let Some(end) = &y.end {
                        end.eval(context_width, &context.variables)
                    } else {
                        Some(beg.clone())
                    };
            ...
            Factor::Unresolved(_, _) | Factor::Unknown(_) => {
