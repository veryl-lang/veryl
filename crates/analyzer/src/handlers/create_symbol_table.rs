...
pub struct CreateSymbolTable {
    ...
    mixin_sources: Vec<GenericSymbolPath>,
    is_public: bool,
...
impl VerylGrammarTrait for CreateSymbolTable {
    ...
    fn inst_declaration(&mut self, arg: &InstDeclaration) -> Result<(), ParolError> {
        if let HandlerPoint::After = self.point {
            ...
            let clock_domain = if let Some(ref x) = inst.component_instantiation_opt {
                ...
                // Interface members inherit this domain, and None is compatible with
                // every domain — default to Implicit (like unannotated vars) so crossings
                // through an interface are still inferred/checked. Module instances ignore it.
                SymClockDomain::Implicit
            } else {
                SymClockDomain::None
            };
    ...
    fn mixin_declaration(&mut self, arg: &MixinDeclaration) -> Result<(), ParolError> {
        if let HandlerPoint::Before = self.point {
            let path: GenericSymbolPath = arg.scoped_identifier.as_ref().into();
            self.mixin_sources.push(path);
        }
        Ok(())
    }
