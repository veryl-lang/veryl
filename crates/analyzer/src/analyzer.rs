...
impl Analyzer {
    ...
    fn create_ir(
        ...
        build_opt: &Build,
        emit_ir_error: bool,
    ) -> (Ir, Vec<AnalyzerError>) {
        let mut context = Context::default();
        context.config.instance_depth_limit = build_opt.instance_depth_limit;
        context.config.instance_total_limit = build_opt.instance_total_limit;
        context.config.evaluate_size_limit = build_opt.evaluate_size_limit;
        context.emit_ir_error = emit_ir_error;

        let ir: IrResult<Ir> = Conv::conv(&mut context, input);
    ...
    pub fn analyze_pass2(
        ...
        namespace_table::set_default(&[project_name.into()]);
        let mut ir_result = Self::create_ir(input, &self.build_opt, ir.is_some());
        if let Some(x) = ir {
