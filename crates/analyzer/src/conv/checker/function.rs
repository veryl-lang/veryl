...
pub fn check_function_call_statement(context: &mut Context, arg: &IdentifierStatement) {
    if let IdentifierStatementGroup::FunctionCall(x) = arg.identifier_statement_group.as_ref() {
...
pub fn check_function_call_factor(context: &mut Context, arg: &IdentifierFactor) {
    // not function call
