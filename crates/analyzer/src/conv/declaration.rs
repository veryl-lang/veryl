...
use crate::conv::checker::connect::check_connect;
...
use crate::conv::utils::{
    TypePosition, eval_array_literal, eval_assign_statement, eval_clock, eval_expr, eval_range,
    eval_reset, eval_struct_constructor, expand_connect, expand_connect_const, get_component,
    get_overridden_params, get_port_connects, get_return_str, insert_port_connect,
};
use crate::conv::{Affiliation, Context, Conv};
use crate::ir::{
    self, Comptime, FuncArg, FuncPath, IrResult, Signature, TypeKind, ValueVariant, VarId,
...
impl Conv<&GenerateForDeclaration> for ir::DeclarationBlock {
    fn conv(context: &mut Context, value: &GenerateForDeclaration) -> IrResult<Self> {
        ...
        for i in beg..end {
            ...
            let comptime = Comptime::create_value(BigUint::from(i), 32, ClockDomain::None, token);
...
impl Conv<&WithParameterItem> for () {
    fn conv(context: &mut Context, value: &WithParameterItem) -> IrResult<Self> {
            ...
            && let SymbolKind::Parameter(x) = symbol.found.kind
        {
            ...
            let comptime = if context.is_affiliated(Affiliation::ProtoModule) {
                Comptime::create_unknown(ClockDomain::None, token)
            } else {
                ...
                let (comptime, _) = eval_expr(context, Some(r#type.clone()), expr, false)?;
                comptime
            };

            // TODO expand for struct
...
impl Conv<&PortDeclarationItem> for () {
    fn conv(context: &mut Context, value: &PortDeclarationItem) -> IrResult<Self> {
            ...
            && let SymbolKind::Port(x) = &symbol.found.kind
        {
            ...
            for (path, r#type) in r#type.expand_struct_middle(&path) {
                let comptime = Comptime::from_type(r#type, clock_domain, variable_token);
                let _ = context.insert_var_path(path, comptime);
            }
...
impl Conv<&LetDeclaration> for ir::Declaration {
    fn conv(context: &mut Context, value: &LetDeclaration) -> IrResult<Self> {
            ...
            && let SymbolKind::Variable(x) = symbol.found.kind
        {
            ...
            let expr = eval_expr(context, Some(r#type.clone()), &value.expression, false)?;

            let statements = eval_assign_statement(context, &dst, &expr, token)?;
...
impl Conv<&ConstDeclaration> for ir::Declaration {
    fn conv(context: &mut Context, value: &ConstDeclaration) -> IrResult<Self> {
            ...
            && let SymbolKind::Parameter(x) = symbol.found.kind
        {
            ...
            let r#type = x.r#type.to_ir_type(context, TypePosition::Variable)?;
            let clock_domain = ClockDomain::None;
            ...
            match expr {
                ir::Expression::ArrayLiteral(_) => {
                    ...
                    if let Some(exprs) = exprs {
                        ...
                        for expr in exprs {
                            let value = expr
                                .eval_value(context, Some(r#type.total_width()))
                ...
                ir::Expression::StructConstructor(_, _) => {
                    let mut dsts = vec![];
                    for x in r#type.expand_struct(&path) {
                        let path = x.path;
                        let r#type = x.r#type;

                        // create dummy dst for eval_struct_constructor
                        let dst = ir::AssignDestination {
                            id: VarId::default(),
                            path: path.clone(),
                            index: VarIndex::default(),
                    ...
                    for (dst, expr) in exprs {
                        let value = expr
                            .eval_value(context, Some(dst.comptime.r#type.total_width()))
                            .ok_or_else(|| ir_error!(token))?;

                        let mut comptime =
                            Comptime::from_type(dst.comptime.r#type.clone(), clock_domain, token);
                        comptime.value = ValueVariant::Numeric(value.clone());
    ...
    fn conv(context: &mut Context, value: &FunctionDeclaration) -> IrResult<Self> {
            ...
            && let SymbolKind::Function(x) = symbol.found.kind
        {
            let ret_type = if let Some(x) = &x.ret {
                ...
                if r#type.is_struct() {
                    r#type.width = vec![r#type.total_width()];
                    ...
                    if let Some((_, comptime)) = c.var_paths.get(&path).cloned() {
                        ...
                        if modport_members.is_empty() {
                            ...
                            for x in comptime.r#type.expand_struct(&path) {
                                if let Some((id, comptime)) = c.var_paths.get(&x.path).cloned() {
                                    let variable =
                                    ...
                                    arg_map.insert(x.path.clone(), id);
                                    members.push((x.path, comptime, direction));
                                }
                            }
                            let arg = FuncArg {
                                name: path.first(),
        ...
        match component? {
            ...
            ir::Component::Interface(component) => {
                ...
                if let Some(x) = &value.component_instantiation_opt0 {
                    ...
                    for expr in exprs {
                        let (comptime, _) = eval_expr(context, None, expr, false)?;
            ...
            ir::Component::SystemVerilog(mut component) => {
                    ...
                    && let Some(x) = &x.inst_port.inst_port_opt
                {
                    ...
                    for port in ports {
                        ...
                        let mut dst_paths: Vec<_> = dst_paths
                            .into_iter()
                            .filter_map(|x| x.to_assign_destination(context, true))
                            .collect();
