...
use crate::conv::utils::{
    TypePosition, eval_assign_statement, eval_clock, eval_const_assign, eval_expr, eval_range,
                    ...
                    for port in ports {
                        ...
                        if let Some((dst_type, clock_domain)) = component.port_types.get(&path) {
                            ...
                            for (path, dst, mut expr) in connects {
                                ...
                                for x in dst_type.expand_struct(&path) {
                                    if let Some(id) = component.ports.get(&x.path)
