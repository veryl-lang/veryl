...
use crate::conv::utils::{
    TypePosition, argument_list, case_condition, eval_expr, eval_size, eval_struct_member,
    eval_type, function_call, range_list, switch_condition,
};
        ...
        if let Some(x) = &value.identifier_factor_opt {
            ...
            match x {
                ...
                IdentifierFactorOptGroup::StructConstructor(x) => {
                    ...
                    let members = match &r#type.kind {
                        TypeKind::Struct(x) => &x.members,
                        TypeKind::Union(x) => &x.members,
                        _ => {
            ...
            let factor = if let Some((var_id, mut comptime)) = found {
                ...
                let width_select = if let Some(part_select) = &comptime.part_select {
                    part_select
                        .conv_width_select(context, width_select)
