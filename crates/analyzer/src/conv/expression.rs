...
use crate::conv::utils::{
    argument_list, case_condition, eval_expr, function_call, range_list, switch_condition,
};
use crate::conv::{Context, Conv};
use crate::ir::{self, Comptime, IrResult, Op, Type, TypeKind, ValueVariant, VarPathSelect};
use crate::ir_error;
use crate::literal::Literal;
use crate::symbol::{EnumMemberValue, GenericBoundKind, ProtoBound, SymbolKind};
        ...
        let value = match value.factor_type_group.as_ref() {
            FactorTypeGroup::VariableTypeFactorTypeOpt(x) => {
                ...
                match x.variable_type.as_ref() {
                    VariableType::Clock(_) => Type::new(TypeKind::Clock, vec![], width, false),
                    VariableType::ClockPosedge(_) => {
                        Type::new(TypeKind::ClockPosedge, vec![], width, false)
                    }
                    VariableType::ClockNegedge(_) => {
                        Type::new(TypeKind::ClockNegedge, vec![], width, false)
                    }
                    VariableType::Reset(_) => Type::new(TypeKind::Reset, vec![], width, false),
                    VariableType::ResetAsyncHigh(_) => {
                        Type::new(TypeKind::ResetAsyncHigh, vec![], width, false)
                    }
                    VariableType::ResetAsyncLow(_) => {
                        Type::new(TypeKind::ResetAsyncLow, vec![], width, false)
                    }
                    VariableType::ResetSyncHigh(_) => {
                        Type::new(TypeKind::ResetSyncHigh, vec![], width, false)
                    }
                    VariableType::ResetSyncLow(_) => {
                        Type::new(TypeKind::ResetSyncLow, vec![], width, false)
                    }
                    VariableType::Logic(_) => Type::new(TypeKind::Logic, vec![], width, false),
                    VariableType::Bit(_) => Type::new(TypeKind::Bit, vec![], width, false),
                }
            }
            FactorTypeGroup::FixedType(x) => match x.fixed_type.as_ref() {
                FixedType::U8(_) => Type::new(TypeKind::Bit, vec![], vec![8], false),
                FixedType::U16(_) => Type::new(TypeKind::Bit, vec![], vec![16], false),
                FixedType::U32(_) => Type::new(TypeKind::Bit, vec![], vec![32], false),
                FixedType::U64(_) => Type::new(TypeKind::Bit, vec![], vec![64], false),
                FixedType::I8(_) => Type::new(TypeKind::Bit, vec![], vec![8], true),
                FixedType::I16(_) => Type::new(TypeKind::Bit, vec![], vec![16], true),
                FixedType::I32(_) => Type::new(TypeKind::Bit, vec![], vec![32], true),
                FixedType::I64(_) => Type::new(TypeKind::Bit, vec![], vec![64], true),
                FixedType::F32(_) => Type::new(TypeKind::Bit, vec![], vec![32], false),
                FixedType::F64(_) => Type::new(TypeKind::Bit, vec![], vec![64], false),
                FixedType::Bool(_) => Type::new(TypeKind::Bit, vec![], vec![1], false),
                FixedType::Strin(_) => Type::new(TypeKind::Unknown, vec![], vec![1], false),
            },
        };
        let r#type = Type {
            kind: TypeKind::Type,
...
impl Conv<&CastingType> for ir::Factor {
    fn conv(context: &mut Context, value: &CastingType) -> IrResult<Self> {
        ...
        let value = match value {
            CastingType::U8(_) => Type::new(TypeKind::Bit, vec![], vec![8], false),
            CastingType::U16(_) => Type::new(TypeKind::Bit, vec![], vec![16], false),
            CastingType::U32(_) => Type::new(TypeKind::Bit, vec![], vec![32], false),
            CastingType::U64(_) => Type::new(TypeKind::Bit, vec![], vec![64], false),
            CastingType::I8(_) => Type::new(TypeKind::Bit, vec![], vec![8], true),
            CastingType::I16(_) => Type::new(TypeKind::Bit, vec![], vec![16], true),
            CastingType::I32(_) => Type::new(TypeKind::Bit, vec![], vec![32], true),
            CastingType::I64(_) => Type::new(TypeKind::Bit, vec![], vec![64], true),
            CastingType::F32(_) => Type::new(TypeKind::Bit, vec![], vec![32], false),
            CastingType::F64(_) => Type::new(TypeKind::Bit, vec![], vec![64], false),
            CastingType::Bool(_) => Type::new(TypeKind::Bit, vec![], vec![1], false),
            CastingType::Clock(_) => Type::new(TypeKind::Clock, vec![], vec![1], false),
            CastingType::ClockPosedge(_) => {
                Type::new(TypeKind::ClockPosedge, vec![], vec![1], false)
            }
            CastingType::ClockNegedge(_) => {
                Type::new(TypeKind::ClockNegedge, vec![], vec![1], false)
            }
            CastingType::Reset(_) => Type::new(TypeKind::Reset, vec![], vec![1], false),
            CastingType::ResetAsyncHigh(_) => {
                Type::new(TypeKind::ResetAsyncHigh, vec![], vec![1], false)
            }
            CastingType::ResetAsyncLow(_) => {
                Type::new(TypeKind::ResetAsyncLow, vec![], vec![1], false)
            }
            CastingType::ResetSyncHigh(_) => {
                Type::new(TypeKind::ResetSyncHigh, vec![], vec![1], false)
            }
            CastingType::ResetSyncLow(_) => {
                Type::new(TypeKind::ResetSyncLow, vec![], vec![1], false)
            }
            ...
            CastingType::Based(x) => {
                ...
                Type::new(TypeKind::Logic, vec![], vec![value], false)
            }
    ...
    fn conv(context: &mut Context, value: &Factor) -> IrResult<Self> {
        ...
        match value {
            ...
            Factor::BooleanLiteral(x) => {
                ...
                let r#type = Type::new(TypeKind::Bit, vec![], vec![1], false);
                let ret = Comptime {
            ...
            Factor::StringLiteral(x) => {
                let text = x.string_literal.string_literal_token.token.text.to_string();
                let r#type = Type {
                ...
                let ret = Comptime {
                    value: ValueVariant::String(text),
                    r#type,
                    is_const: true,
                    is_global: true,
                    token,
            ...
            Factor::FactorGroup(x) => match x.factor_group.as_ref() {
                FactorGroup::Msb(_) => {
                    ...
                    let (_, comptime) = context
                        .var_paths
                        .get(path)
                        .ok_or_else(|| ir_error!(token))?;

                    let dim = context.get_select_dim().unwrap();
                    let width = comptime.r#type.width[dim];
                    let msb = width - 1;
                    Ok(ir::Expression::Term(Box::new(ir::Factor::Value(
                ...
                FactorGroup::Lsb(_) => Ok(ir::Expression::Term(Box::new(ir::Factor::Value(
                    Comptime::create_value(0u32.into(), 32, token),
                    token,
                )))),
            },
            ...
            Factor::TypeExpression(x) => {
                let (comptime, _) = eval_expr(context, None, &x.type_expression.expression)?;
                let r#type = Type {
                    kind: TypeKind::Type,
                    signed: false,
                    width: vec![],
...
impl Conv<&IdentifierFactor> for ir::Expression {
    fn conv(context: &mut Context, value: &IdentifierFactor) -> IrResult<Self> {
        ...
        if let Some(x) = &value.identifier_factor_opt {
            ...
            let factor = if let Some((var_id, mut comptime)) = context.find_path(&path) {
                let (array_select, width_select) = select.split(comptime.r#type.array.len());

                // Array select type check
                let _ = array_select.eval_comptime(context, &comptime.r#type.array, true);
                ...
                let path = context.resolve_path(x.into());
                if let Some(x) = path.to_literal() {
                    let (value, r#type) = match x {
                        ...
                        Literal::Type(x) => {
                            let r#type = ir::Type::new(ir::TypeKind::Type, vec![], vec![1], false);
                        ...
                        Literal::String(x) => {
                            let r#type =
                                ir::Type::new(ir::TypeKind::String, vec![], vec![1], false);
                            (ValueVariant::String(x), r#type)
                        }
                    };
                    let x = Comptime {
                        value,
                        ...
                        SymbolKind::GenericParameter(x) => {
                                ...
                                x.bound.resolve_proto_bound(&symbol.found.namespace)
                            {
                                ...
                                if let Some(r#type) = r#type {
                                    let r#type = r#type.to_ir_type(context)?;
                            ...
                            if let Some(width) = x.width {
                                match &x.value {
                                    EnumMemberValue::ImplicitValue(x) => {
                                        let x = Comptime::create_value((*x).into(), width, token);

                                        return Ok(ir::Expression::Term(Box::new(
                                            ir::Factor::Value(x, token),
                                        )));
                                    }
                                    EnumMemberValue::ExplicitValue(x, _) => {
                                        let (x, _) = eval_expr(context, None, x)?;
                                        return Ok(ir::Expression::Term(Box::new(
                                            ir::Factor::Value(x, token),
                                        )));
                                    }
...
impl Conv<&Based> for Comptime {
    fn conv(_context: &mut Context, value: &Based) -> IrResult<Self> {
        let token: TokenRange = value.into();
        ...
        let r#type = Type::new(kind, vec![], vec![value.width], value.signed);
...
impl Conv<&BaseLess> for Comptime {
    fn conv(_context: &mut Context, value: &BaseLess) -> IrResult<Self> {
        let token: TokenRange = value.into();
        let value: Value = value.into();
        let r#type = Type::new(TypeKind::Bit, vec![], vec![32], true);
...
impl Conv<&AllBit> for Comptime {
    fn conv(_context: &mut Context, value: &AllBit) -> IrResult<Self> {
        ...
        let r#type = Type::new(kind, vec![], vec![0], true);
    ...
    fn conv(_context: &mut Context, value: &RealNumber) -> IrResult<Self> {
        ...
        let ret = match value {
            RealNumber::FixedPoint(x) => {
                ...
                let r#type = Type::new(TypeKind::Bit, vec![], vec![64], false);
