...
use crate::conv::utils::{
    TypePosition, argument_list, case_condition, eval_expr, eval_struct_member, eval_type,
    function_call, range_list, switch_condition,
};
use crate::conv::{Context, Conv};
use crate::ir::{
    self, Comptime, IrResult, Op, Type, TypeKind, ValueVariant, VarPath, VarPathSelect,
};
use crate::literal::Literal;
use crate::symbol::{self, ClockDomain, EnumMemberValue, GenericBoundKind, ProtoBound, SymbolKind};
use crate::symbol_path::GenericSymbolPath;
use crate::symbol_table;
use crate::value::Value;
use crate::{HashMap, ir_error, msb_table};
    ...
    fn conv(context: &mut Context, value: &FactorType) -> IrResult<Self> {
        ...
        let value = match value.factor_type_group.as_ref() {
            FactorTypeGroup::VariableTypeFactorTypeOpt(x) => {
                ...
                Type::new(kind, vec![], width, false)
            }
            FactorTypeGroup::FixedType(x) => {
                let (kind, width, signed) = match x.fixed_type.as_ref() {
                    FixedType::U8(_) => (TypeKind::Bit, vec![8], false),
                    FixedType::U16(_) => (TypeKind::Bit, vec![16], false),
                    FixedType::U32(_) => (TypeKind::Bit, vec![32], false),
                    FixedType::U64(_) => (TypeKind::Bit, vec![64], false),
                    FixedType::I8(_) => (TypeKind::Bit, vec![8], true),
                    FixedType::I16(_) => (TypeKind::Bit, vec![16], true),
                    FixedType::I32(_) => (TypeKind::Bit, vec![32], true),
                    FixedType::I64(_) => (TypeKind::Bit, vec![64], true),
                    FixedType::F32(_) => (TypeKind::Bit, vec![32], false),
                    FixedType::F64(_) => (TypeKind::Bit, vec![64], false),
                    FixedType::Bool(_) => (TypeKind::Bit, vec![1], false),
                    FixedType::Strin(_) => (TypeKind::Unknown, vec![1], false),
                };
                Type::new(kind, vec![], width, signed)
            }
        };
        let r#type = Type {
            kind: TypeKind::Type,
    ...
    fn conv(context: &mut Context, value: &CastingType) -> IrResult<Self> {
        ...
        let value = if let CastingType::UserDefinedType(x) = value {
            ...
            let (kind, width, signed) = match value {
                CastingType::U8(_) => (TypeKind::Bit, vec![8], false),
                CastingType::U16(_) => (TypeKind::Bit, vec![16], false),
                CastingType::U32(_) => (TypeKind::Bit, vec![32], false),
                CastingType::U64(_) => (TypeKind::Bit, vec![64], false),
                CastingType::I8(_) => (TypeKind::Bit, vec![8], true),
                CastingType::I16(_) => (TypeKind::Bit, vec![16], true),
                CastingType::I32(_) => (TypeKind::Bit, vec![32], true),
                CastingType::I64(_) => (TypeKind::Bit, vec![64], true),
                CastingType::F32(_) => (TypeKind::Bit, vec![32], false),
                CastingType::F64(_) => (TypeKind::Bit, vec![64], false),
                CastingType::Bool(_) => (TypeKind::Bit, vec![1], false),
                CastingType::Clock(_) => (TypeKind::Clock, vec![1], false),
                CastingType::ClockPosedge(_) => (TypeKind::ClockPosedge, vec![1], false),
                CastingType::ClockNegedge(_) => (TypeKind::ClockNegedge, vec![1], false),
                CastingType::Reset(_) => (TypeKind::Reset, vec![1], false),
                CastingType::ResetAsyncHigh(_) => (TypeKind::ResetAsyncHigh, vec![1], false),
                CastingType::ResetAsyncLow(_) => (TypeKind::ResetAsyncLow, vec![1], false),
                CastingType::ResetSyncHigh(_) => (TypeKind::ResetSyncHigh, vec![1], false),
                CastingType::ResetSyncLow(_) => (TypeKind::ResetSyncLow, vec![1], false),
                CastingType::Based(x) => {
                    let value: Comptime = Conv::conv(context, x.based.as_ref())?;
                    let value = value.get_value().unwrap().to_usize();
                    (TypeKind::Bit, vec![value], false)
                }
            ...
            Factor::FactorGroup(x) => {
                let Some((path, generic_path)) = context.select_paths.last().cloned() else {
                    match x.factor_group.as_ref() {
                        ...
                        FactorGroup::Lsb(_) => {
                            context.insert_error(AnalyzerError::invalid_lsb(&token));
                        }
                    }
                ...
                match x.factor_group.as_ref() {
                    FactorGroup::Msb(msb) => {
                        if let Some((_, comptime)) = context.find_path(&path) {
                            ...
                            if comptime.r#type.is_struct() {
                                msb_table::insert(msb.msb.msb_token.token.id, 0);
                            } else {
                            ...
                            let msb = width - 1;
                            Ok(ir::Expression::Term(Box::new(ir::Factor::Value(
                                Comptime::create_value(msb.into(), 32, ClockDomain::None, token),
                                token,
                            ))))
                    ...
                    FactorGroup::Lsb(_) => Ok(ir::Expression::Term(Box::new(ir::Factor::Value(
                        Comptime::create_value(0u32.into(), 32, ClockDomain::None, token),
                        token,
                    )))),
                }
            }
        ...
        if let Some(x) = &value.identifier_factor_opt {
            ...
            match x {
                IdentifierFactorOptGroup::FunctionCall(x) => {
                    ...
                    match &symbol.found.kind {
                        ...
                        _ => {
                            ...
                            context.insert_error(AnalyzerError::call_non_function(
                                &name, &kind, &token,
                            ));
                            Err(ir_error!(token))
                        }
                ...
                IdentifierFactorOptGroup::StructConstructor(x) => {
                    let items: Vec<_> =
                        x.struct_constructor.struct_constructor_list.as_ref().into();
                    ...
                    let r#type = symbol::Type {
                        modifier: vec![],
                        kind: symbol::TypeKind::UserDefined(symbol::UserDefinedType {
                            path,
                            symbol: None,
                        }),
                        width: vec![],
                    ...
                    if let TypeKind::Struct(x) = &r#type.kind {
            ...
            let factor = if let Some((var_id, mut comptime)) = found {
                ...
                if array_select.is_range() {
                    ...
                    if comptime.r#type.is_struct() {
                        let mut ids = vec![];
                        for x in comptime.r#type.expand_struct(&path) {
                            let (id, _) =
                                context.find_path(&x.path).ok_or_else(|| ir_error!(token))?;
                            ids.push(id);
                        }
                        ir::Factor::Struct(ids, index, width_select, comptime, token)
                    } else if comptime.r#type.is_type() {
                    ...
                    match &symbol.found.kind {
                        SymbolKind::Parameter(x) => {
                            if let Some(expr) = &x.value {
                                ...
                                context.push_generic_map(path.to_generic_maps());
                                let ret =
                        ...
                        SymbolKind::GenericParameter(x) => {
                                ...
                                x.bound.resolve_proto_bound(&symbol.found.namespace)
                            {
                                ...
                                if let Some(r#type) = r#type {
                                    let r#type =
                                        r#type.to_ir_type(context, TypePosition::Generic)?;
                        ...
                        SymbolKind::ProtoConst(_) | SymbolKind::ProtoTypeDef(_) => {
                            let mut x = Comptime::create_unknown(ClockDomain::None, token);
                        ...
                        SymbolKind::Enum(_) | SymbolKind::Struct(_) | SymbolKind::Union(_) => {
                            ...
                            let x = Comptime {
                                ...
                                r#type: ir::Type::new(ir::TypeKind::Type, vec![], vec![], false),
                                is_const: true,
                                is_global: true,
                                clock_domain: ClockDomain::None,
                                token,
