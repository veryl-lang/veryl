...
use crate::conv::utils::{argument_list, case_condition, eval_expr, range_list, switch_condition};
use crate::conv::{Context, Conv};
use crate::ir::bigint::gen_mask;
use crate::ir::{self, Op, Type, TypeKind, TypedValue, Value, ValueVariant, VarPathSelect};
...
impl Conv<&FactorType> for ir::Factor {
    fn conv(context: &mut Context, value: &FactorType) -> Self {
        ...
        let value = match value.factor_type_group.as_ref() {
            FactorTypeGroup::VariableTypeFactorTypeOpt(x) => {
                ...
                match x.variable_type.as_ref() {
                    VariableType::Clock(_) => Type::new(TypeKind::Clock, width, false),
                    VariableType::ClockPosedge(_) => {
                        Type::new(TypeKind::ClockPosedge, width, false)
                    }
                    VariableType::ClockNegedge(_) => {
                        Type::new(TypeKind::ClockNegedge, width, false)
                    }
                    VariableType::Reset(_) => Type::new(TypeKind::Reset, width, false),
                    VariableType::ResetAsyncHigh(_) => {
                        Type::new(TypeKind::ResetAsyncHigh, width, false)
                    }
                    VariableType::ResetAsyncLow(_) => {
                        Type::new(TypeKind::ResetAsyncLow, width, false)
                    }
                    VariableType::ResetSyncHigh(_) => {
                        Type::new(TypeKind::ResetSyncHigh, width, false)
                    }
                    VariableType::ResetSyncLow(_) => {
                        Type::new(TypeKind::ResetSyncLow, width, false)
                    }
                    VariableType::Logic(_) => Type::new(TypeKind::Logic, width, false),
                    VariableType::Bit(_) => Type::new(TypeKind::Bit, width, false),
                }
            }
            FactorTypeGroup::FixedType(x) => match x.fixed_type.as_ref() {
                FixedType::U8(_) => Type::new(TypeKind::Bit, vec![8], false),
                FixedType::U16(_) => Type::new(TypeKind::Bit, vec![16], false),
                FixedType::U32(_) => Type::new(TypeKind::Bit, vec![32], false),
                FixedType::U64(_) => Type::new(TypeKind::Bit, vec![64], false),
                FixedType::I8(_) => Type::new(TypeKind::Bit, vec![8], true),
                FixedType::I16(_) => Type::new(TypeKind::Bit, vec![16], true),
                FixedType::I32(_) => Type::new(TypeKind::Bit, vec![32], true),
                FixedType::I64(_) => Type::new(TypeKind::Bit, vec![64], true),
                FixedType::F32(_) => Type::new(TypeKind::Bit, vec![32], false),
                FixedType::F64(_) => Type::new(TypeKind::Bit, vec![64], false),
                FixedType::Bool(_) => Type::new(TypeKind::Bit, vec![1], false),
                FixedType::Strin(_) => Type::new(TypeKind::Unknown, vec![1], false),
            },
        };
        let r#type = Type {
            kind: TypeKind::Type,
...
impl Conv<&CastingType> for ir::Factor {
    fn conv(context: &mut Context, value: &CastingType) -> Self {
        ...
        let value = match value {
            CastingType::U8(_) => Type::new(TypeKind::Bit, vec![8], false),
            CastingType::U16(_) => Type::new(TypeKind::Bit, vec![16], false),
            CastingType::U32(_) => Type::new(TypeKind::Bit, vec![32], false),
            CastingType::U64(_) => Type::new(TypeKind::Bit, vec![64], false),
            CastingType::I8(_) => Type::new(TypeKind::Bit, vec![8], true),
            CastingType::I16(_) => Type::new(TypeKind::Bit, vec![16], true),
            CastingType::I32(_) => Type::new(TypeKind::Bit, vec![32], true),
            CastingType::I64(_) => Type::new(TypeKind::Bit, vec![64], true),
            CastingType::F32(_) => Type::new(TypeKind::Bit, vec![32], false),
            CastingType::F64(_) => Type::new(TypeKind::Bit, vec![64], false),
            CastingType::Bool(_) => Type::new(TypeKind::Bit, vec![1], false),
            CastingType::Clock(_) => Type::new(TypeKind::Clock, vec![1], false),
            CastingType::ClockPosedge(_) => Type::new(TypeKind::ClockPosedge, vec![1], false),
            CastingType::ClockNegedge(_) => Type::new(TypeKind::ClockNegedge, vec![1], false),
            CastingType::Reset(_) => Type::new(TypeKind::Reset, vec![1], false),
            CastingType::ResetAsyncHigh(_) => Type::new(TypeKind::ResetAsyncHigh, vec![1], false),
            CastingType::ResetAsyncLow(_) => Type::new(TypeKind::ResetAsyncLow, vec![1], false),
            CastingType::ResetSyncHigh(_) => Type::new(TypeKind::ResetSyncHigh, vec![1], false),
            CastingType::ResetSyncLow(_) => Type::new(TypeKind::ResetSyncLow, vec![1], false),
            CastingType::UserDefinedType(x) => {
                let kind = if let Ok(symbol) =
                    symbol_table::resolve(x.user_defined_type.scoped_identifier.as_ref())
                {
                    TypeKind::UserDefined(symbol.found.id)
                } else {
                    TypeKind::Unknown
                };
                Type {
            ...
            CastingType::Based(x) => {
                ...
                Type::new(TypeKind::Logic, vec![value], false)
            }
    ...
    fn conv(context: &mut Context, value: &Factor) -> Self {
        ...
        match value {
            Factor::Number(x) => {
                ...
                ir::Expression::Term(Box::new(ir::Factor::Value(x, token)))
            }
            Factor::BooleanLiteral(x) => {
                ...
                let r#type = Type::new(TypeKind::Bit, vec![1], false);
                let ret = TypedValue {
                ...
                ir::Expression::Term(Box::new(ir::Factor::Value(ret, token)))
            }
            ...
            Factor::QuoteLBraceArrayLiteralListRBrace(x) => {
                ...
                ir::Expression::ArrayLiteral(ret)
            }
            ...
            Factor::FactorGroup(x) => match x.factor_group.as_ref() {
                FactorGroup::Msb(_) => {
                    if let Some(path) = context.select_paths.last() {
                        if let Some((_, typed_value)) = context.var_paths.get(path) {
                            let dim = context.get_select_dim().unwrap();
                            let width = typed_value.r#type.width[dim];
                            let msb = width - 1;
                            ir::Expression::Term(Box::new(ir::Factor::Value(
                                TypedValue::create_value(msb.into(), 32),
                                token,
                            )))
                        } else {
                            context.insert_error(AnalyzerError::unsupported_by_ir(&token));
                        ...
                        context.insert_error(AnalyzerError::invalid_msb(&token));
                        ir::Expression::Term(Box::new(ir::Factor::Unknown(token)))
                    }
                }
                FactorGroup::Lsb(_) => ir::Expression::Term(Box::new(ir::Factor::Value(
                    TypedValue::create_value(0u32.into(), 32),
                    token,
                ))),
            },
...
impl Conv<&IdentifierFactor> for ir::Expression {
    fn conv(context: &mut Context, value: &IdentifierFactor) -> Self {
        ...
        if let Some(x) = &value.identifier_factor_opt {
            ...
            if let IdentifierFactorOptGroup::FunctionCall(x) = x {
                ...
                let Ok(symbol) = symbol_table::resolve(value.expression_identifier.as_ref()) else {
                    context.insert_error(AnalyzerError::unsupported_by_ir(&token));
                    return ir::Expression::Term(Box::new(ir::Factor::Unknown(token)));
                };
                ...
                match &symbol.found.kind {
                    SymbolKind::SystemFunction(_) => {
                        ...
                        let token: TokenRange = value.into();
                        let typed_values: Vec<_> = args
                            .iter()
                            .map(|x: &ir::Expression| x.eval_type(context, None))
                            .collect();
                        kind.eval_type(context, &typed_values, &token);

                        let ret = ir::SystemFunctionCall { kind, args };
                        ir::Expression::Term(Box::new(ir::Factor::SystemFunctionCall(ret, token)))
                    }
                    SymbolKind::Function(_) => {
                        ...
                        // TODO select for function member of arrayed instance
                        ...
                        if let Some((id, ret)) = context.func_paths.get(&path) {
                            ...
                            ir::Expression::Term(Box::new(ir::Factor::FunctionCall(ret, token)))
                        } else {
                            context.insert_error(AnalyzerError::unsupported_by_ir(&token));
                            ir::Expression::Term(Box::new(ir::Factor::Unknown(token)))
                        }
                    ...
                    SymbolKind::ModportFunctionMember(_) => {
                        let _ = args.to_function_args(context, &symbol.found);
                        context.insert_error(AnalyzerError::unsupported_by_ir(&token));
                        ir::Expression::Term(Box::new(ir::Factor::Unknown(token)))
                    }
                    _ => {
            ...
            let factor = if let Some((var_id, mut typed_value)) = context.find_path(&path) {
                let (array_select, width_select) = select.split(typed_value.r#type.array.len());

                // Array select type check
                let _ = array_select.eval_type(context, &typed_value.r#type.array, true);

                typed_value.is_global = false;

                if array_select.is_range() {
                    // TODO
                    context.insert_error(AnalyzerError::unsupported_by_ir(&token));
                    ir::Factor::Unknown(token)
                } else {
                    let index = array_select.to_index();
                    ir::Factor::Variable(var_id, index, width_select, typed_value, token)
                }
            } else {
                ...
                if let Ok(symbol) = symbol_table::resolve(path) {
                    match &symbol.found.kind {
                        SymbolKind::Parameter(x) => {
                            if let Some(r#type) = x.r#type.to_ir_type(context)
                                && let Some(expr) = &x.value
                            {
                                let (x, _) = eval_expr(context, Some(r#type), expr);
                                return ir::Expression::Term(Box::new(ir::Factor::Value(x, token)));
                            }
                        }
                        SymbolKind::GenericParameter(x) => {
                            //let name = symbol.found.token.text;
                            //if let Some(x) = context.get_generic_argument(&name) {
                            //    let path: GenericSymbolPathNamesapce =
                            //        (x, &symbol.found.namespace).into();
                            //    if let Ok(symbol) = symbol_table::resolve(&path) {
                            //    }
                            //} else {
                            if let Some(proto) =
                                x.bound.resolve_proto_bound(&symbol.found.namespace)
                            {
                                ...
                                if let Some(r#type) = r#type
                                    && let Some(r#type) = r#type.to_ir_type(context)
                                {
                                    let mut x = TypedValue::create_unknown();

                                    // GenericParameter is global
                                    x.is_global = true;
                                    x.r#type = r#type;
                    ...
                    ir::Factor::Unresolved(x.clone(), token)
                } else {
                    ir::Factor::Unresolved(x.clone(), token)
                }
            };
...
impl Conv<&Based> for TypedValue {
    fn conv(_context: &mut Context, value: &Based) -> Self {
        ...
        if let Some((width, rest)) = x.split_once('\'') {
            ...
            let r#type = Type::new(kind, vec![width], signed);
...
impl Conv<&BaseLess> for TypedValue {
    fn conv(_context: &mut Context, value: &BaseLess) -> Self {
        ...
        let r#type = Type::new(TypeKind::Bit, vec![32], true);
...
impl Conv<&AllBit> for TypedValue {
    fn conv(_context: &mut Context, value: &AllBit) -> Self {
        ...
        let r#type = Type::new(kind, vec![0], true);
    ...
    fn conv(_context: &mut Context, value: &RealNumber) -> Self {
        match value {
            RealNumber::FixedPoint(x) => {
                ...
                let r#type = Type::new(TypeKind::Bit, vec![64], false);
...
impl Conv<&AssignmentOperator> for Op {
    fn conv(_context: &mut Context, value: &AssignmentOperator) -> Self {
