...
use crate::ir::{
    self, Arguments, Comptime, FuncPath, FuncProto, IrResult, Op, Shape, ShapeRef, Signature,
    ValueVariant, VarId, VarIndex, VarKind, VarPath, VarPathSelect, VarSelect, Variable,
};
use crate::symbol::{Affiliation, ClockDomain, GenericBoundKind, SymbolKind};
...
pub fn eval_array_literal(
    ...
    if let Some(array) = context_array {
        ...
        for x in x {
            ...
            let next_array: Option<&ShapeRef> = if array.dims() < 2 {
                None
            } else {
                Some(array[1..].into())
            };
        ...
        if let Some(Some(target_len)) = array.get(0) {
...
pub fn eval_size(context: &mut Context, expr: &mut ir::Expression) -> (Comptime, Option<usize>) {
    ...
    if let Ok(x) = comptime.get_value() {
        ...
        context.check_size(value, expr.token_range());
        (comptime, Some(value))
    } else {
        (comptime, None)
    }
...
pub fn eval_assign_statement(
    ...
    if let Some(exprs) = array_exprs {
        ...
        for (dst, mut expr) in exprs.into_iter() {
            check_reset_non_elaborative(context, &mut expr);
    ...
    match expr {
        ir::Expression::ArrayLiteral(_) => {
            ...
            if let Some(exprs) = exprs {
                ...
                for expr in exprs {
                    let value = expr
                        .eval_value(context, r#type.total_width())
        ...
        ir::Expression::StructConstructor(_, _) => {
            ...
            for (dst, expr) in exprs {
                let value = expr
                    .eval_value(context, dst.comptime.r#type.total_width())
                    .ok_or_else(|| ir_error!(token))?;

                let mut comptime =
                    Comptime::from_type(dst.comptime.r#type.clone(), ClockDomain::None, token);
        ...
        _ => {
            match &comptime.value {
                ValueVariant::Numeric(value) => {
                    if let ir::TypeKind::Struct(_) = &comptime.r#type.kind {
                        for x in r#type.expand_struct(path) {
                            let path = x.path;
                            let r#type = x.r#type;
...
pub fn var_path_to_assign_destination(
    ...
    path.into_iter()
        .flat_map(|x| {
            ...
            if let Some(dst) = dst {
                let Ok(mut dst) = dst.expand(context) else {
                    return vec![];
                };
                ret.append(&mut dst);
            }
