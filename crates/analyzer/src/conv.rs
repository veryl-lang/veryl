...
use crate::ir::{
    Comptime, Declaration, FuncPath, Function, Interface, IrResult, Type, ValueVariant, VarId,
    VarIndex, VarKind, VarPath, Variable,
};
use crate::namespace_table;
use crate::symbol::{Affiliation, Direction, GenericMap};
...
pub struct Context {
    ...
    pub hierarchical_functions: Vec<Vec<FuncPath>>,
...
impl Context {
    pub fn inherit(&mut self, tgt: &mut Context) {
        ...
        self.emit_ir_error = tgt.emit_ir_error;
    }
    ...
    pub fn resolve_path(&self, mut path: GenericSymbolPath) -> GenericSymbolPath {
        if let Some(map) = self.generic_maps.last() {
            let namespace = namespace_table::get(path.paths[0].base.id).unwrap();
            path.resolve_imported(&namespace, Some(map));
            path.apply_map(map);
        }
        path
    }
    ...
    pub fn insert_func_path(&mut self, path: FuncPath, value: Option<Comptime>) -> VarId {
        let id = self.var_id;
    ...
    pub fn insert_port_type(&mut self, path: VarPath, r#type: Type) {
        self.port_types.insert(path, r#type);
    }
    ...
    pub fn extract_function(&mut self, context: &mut Context, base: &VarPath, array: &[usize]) {
        ...
        for (mut path, (id, comptime)) in context.func_paths.drain() {
            path.add_prelude(&base.0);
            self.func_paths.insert(path, (id, comptime));
        }
        ...
        for (id, mut function) in context.functions.drain() {
            ...
            if !array.is_empty() {
                let total_array = array.iter().product();
    ...
    pub fn extract_var_paths(&mut self, context: &Context, base: &VarPath, array: &[usize]) {
    ...
    pub fn extract_interface_member(
        ...
        array: &[usize],
        component: Interface,
        modport: Option<&HashMap<StrId, (VarPath, Direction)>>,
        token: TokenRange,
    ) {
        let mut inserted = HashSet::default();

        for mut variable in component.variables.into_values() {
            if let Some(x) = modport {
                if let Some(x) = x.get(&variable.path.0[0]) {
                    variable.kind = match &x.1 {
            ...
            let comptime = Comptime::from_type(variable.r#type.clone(), token);
        ...
        for (mut path, (_, mut comptime)) in component.var_paths {
    ...
    pub fn drain_func_paths(&mut self) -> HashMap<FuncPath, (VarId, Option<Comptime>)> {
    ...
    pub fn drain_port_types(&mut self) -> HashMap<VarPath, Type> {
