...
/// For the tests that target the simulator's own combinational-loop detector.
/// The analyzer rejects such a design first, so nothing else may wave that
/// error through: a test that needs to is testing a design Veryl does not
/// accept.
#[track_caller]
