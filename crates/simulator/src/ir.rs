mod declaration;
mod event;
mod expression;
mod statement;
mod variable;

pub use crate::conv::build_ir;
pub use declaration::{Clock, Declaration, Reset};
pub use event::Event;
pub use expression::{Expression, ExpressionProto};
pub use statement::{
    AssignDestination, AssignStatement, AssignStatementProto, IfStatement, IfStatementProto,
    Statement, StatementProto,
};
pub use variable::Variable;
pub use veryl_analyzer::ir::{Op, VarId, VarPath};
//pub use veryl_analyzer::value::Value;
pub use usize as Value;

use crate::HashMap;
use indent::indent_all_by;
use memmap2::Mmap;
//use rayon::prelude::*;
use std::fmt;
use veryl_analyzer::ir::Function;
use veryl_parser::resource_table::StrId;

#[derive(Default)]
pub struct Ir {
    pub name: StrId,
    pub ports: HashMap<VarPath, VarId>,
    pub values: Vec<Value>,
    pub variables: HashMap<VarId, Variable>,
    pub functions: HashMap<VarId, Function>,
    pub binary: Vec<Mmap>,
    pub declarations: Vec<Declaration>,
}

impl Ir {
    pub fn eval_step(&self, event: &Event) {
        //let _: Vec<_> = self
        //    .declarations
        //    .par_iter()
        //    .with_min_len(1000)
        //    .map(|x: &Declaration| x.eval_step(event))
        //    .collect();

        for x in &self.declarations {
            x.eval_step(event);
        }
    }

    pub fn dump_variables(&self) -> String {
        let mut ret = String::new();

        let mut variables: Vec<_> = self.variables.iter().collect();
        variables.sort_by(|a, b| a.0.cmp(b.0));

        for (_, x) in variables {
            ret.push_str(&format!("{}\n", x));
        }

        ret
    }
}

impl fmt::Display for Ir {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let mut ret = format!("module {} {{\n", self.name);

        let mut variables: Vec<_> = self.variables.iter().collect();
        variables.sort_by(|a, b| a.0.cmp(b.0));

        for (_, x) in variables {
            let text = format!("{}\n", x);
            ret.push_str(&indent_all_by(2, text));
        }

        ret.push('\n');

        for x in &self.declarations {
            let text = format!("{}\n", x);
            ret.push_str(&indent_all_by(2, text));
        }

        ret.push('}');
        ret.fmt(f)
    }
}
