...
use crate::ir::{
    AssignStatement, Event, Expression, ExpressionContext, IfStatement, Op, Statement,
};
...
impl ProtoAssignStatement {
    pub fn build_binary(&self, context: &mut Context, builder: &mut FunctionBuilder) -> Option<()> {
        ...
        if let Some((beg, end)) = self.select {
            ...
            let org = builder
                .ins()
                .load(I64, load_mem_flag, base_addr, self.dst_offset);
            ...
            if let Some(mask_xz) = mask_xz {
                ...
                let org = builder
                    .ins()
                    .load(I64, load_mem_flag, base_addr, self.dst_offset + 8);
                let org = builder.ins().band_imm(org, !mask as i64);
...
impl ProtoIfStatement {
    pub fn build_binary(
        ...
        if let Some(x) = &self.cond {
            // TODO 4-state
            let (cond_payload, _cond_mask_xz) = x.build_binary(context, builder)?;
            builder
                .ins()
...
pub enum ProtoExpression {
    Variable(i32, bool, Option<(usize, usize)>),
    Value(u64, Option<u64>),
    Unary(Op, Box<ProtoOperand>, ExpressionContext),
    Binary(Box<ProtoOperand>, Op, Box<ProtoOperand>, ExpressionContext),
}
...
impl ProtoExpression {
    pub fn build_binary(
        ...
        match self {
            ...
            ProtoExpression::Variable(offset, is_ff, select) => {
                let load_mem_flag = MemFlags::trusted().with_readonly();
                ...
                let mut mask_xz = if context.config.use_4state {
                    let mask_xz = builder
                        .ins()
            ...
            ProtoExpression::Unary(op, x, expr_context) => {
                let (mut x_payload, mut x_mask_xz) = x.expr.build_binary(context, builder)?;
            ...
            ProtoExpression::Binary(x, op, y, expr_context) => {
                let (mut x_payload, mut x_mask_xz) = x.expr.build_binary(context, builder)?;
                let (mut y_payload, mut y_mask_xz) = y.expr.build_binary(context, builder)?;
